...
class SkillRepositoryImplTest {
    ...
    void save() {
        boolean actualResult = skillRepository.save(
                new Skill(UUID.randomUUID(), "description text", "skill name", SkillType.BEGINNER)
        );
    ...
    void update() {
       boolean actualResult = skillRepository.update(
               new Skill(UUID.fromString("6969aa0e-b05b-434c-bdfa-1d67e8872310"), "abc descp", "skill text", SkillType.INTERMEDIATE)
       );
       boolean expectedResult = true;
       assertEquals(expectedResult, actualResult);
    }
